...
public class CompositeReadableBufferTest {
    ...
    public void testPutReadableBuffer() {
        ...
        ReadableBuffer source = ReadableBuffer.ByteBufferReader.wrap(ByteBuffer.wrap(data));

        assertEquals(0, source.position());
        assertEquals(10, source.limit());
        assertEquals(10, source.capacity());

        buffer.put(source);

        assertEquals(10, source.position());
        assertEquals(10, source.limit());
        assertEquals(10, source.capacity());
        ...
        assertTrue(buffer.hasArray());
        assertNotSame(source.array(), buffer.array());
    }

    @Test
    public void testPutReadableBufferWithoutArray() {
        CompositeReadableBuffer buffer = new CompositeReadableBuffer();
        byte[] data = new byte[] {9, 8, 7, 6, 5, 4, 3, 2, 1, 0};

        ByteBuffer nonArrayBuffer = ByteBuffer.wrap(data).asReadOnlyBuffer();
        assertFalse(nonArrayBuffer.hasArray());

        ReadableBuffer source = ReadableBuffer.ByteBufferReader.wrap(nonArrayBuffer);
    ...
    @Test
    public void testPutEmptyReadableBuffer() {
        CompositeReadableBuffer buffer = new CompositeReadableBuffer();

        ReadableBuffer source = ReadableBuffer.ByteBufferReader.wrap(ByteBuffer.wrap(new byte[0]));
    ...
    @Test
    public void testPutReadableBufferWhenReadOnly() {
        CompositeReadableBuffer buffer = new CompositeReadableBuffer();
        CompositeReadableBuffer slice = buffer.slice();

        ReadableBuffer source = ReadableBuffer.ByteBufferReader.wrap(ByteBuffer.wrap(new byte[] { 1 }));

        try {
            slice.put(source);
            fail("Should enforce slice is read-only and throw ReadOnlyBufferException");
        } catch (ReadOnlyBufferException roe) {}
    }
