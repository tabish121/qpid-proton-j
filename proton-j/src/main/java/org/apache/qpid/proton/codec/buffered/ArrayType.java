...
public class ArrayType implements PrimitiveType<Object[]> {
    ...
    private final ArrayEncoding shortArrayEncoding;
    private final ArrayEncoding arrayEncoding;

    public ArrayType(ProtonBufferEncoderImpl encoder, final ProtonBufferDecoderImpl decoder, BooleanType boolType, ByteType byteType, ShortType shortType, IntegerType intType,
        LongType longType, FloatType floatType, DoubleType doubleType, CharacterType characterType) {
        ...
        this.arrayEncoding = new AllArrayEncoding(encoder, decoder);
        this.shortArrayEncoding = new ShortArrayEncoding(encoder, decoder);
    ...
    public ArrayEncoding getEncoding(final Object[] val) {
        TypeEncoding encoder = calculateEncoder(val, this.encoder);
        int size = calculateSize(val, encoder);
        ArrayEncoding arrayEncoding = (val.length > 255 || size > 254) ? this.arrayEncoding : shortArrayEncoding;
        arrayEncoding.setValue(val, encoder, size);
        return arrayEncoding;
    }

    private static TypeEncoding calculateEncoder(final Object[] val, final ProtonBufferEncoderImpl encoder) {
    ...
    public Collection<ArrayEncoding> getAllEncodings() {
        return Arrays.asList(shortArrayEncoding, arrayEncoding);
    }
    ...
    private ArrayEncoding getEncoding(final boolean[] a) {
        return a.length < 254 || a.length <= 255 && allSameValue(a) ? shortArrayEncoding : arrayEncoding;
    }
    ...
    private ArrayEncoding getEncoding(final byte[] a) {
        return a.length < 254 ? shortArrayEncoding : arrayEncoding;
    }
    ...
    private ArrayEncoding getEncoding(final short[] a) {
        return a.length < 127 ? shortArrayEncoding : arrayEncoding;
    }
    ...
    private ArrayEncoding getEncoding(final int[] a) {
        return a.length < 63 || (a.length < 254 && allSmallInts(a)) ? shortArrayEncoding : arrayEncoding;
    }
    ...
    private ArrayEncoding getEncoding(final long[] a) {
        return a.length < 31 || (a.length < 254 && allSmallLongs(a)) ? shortArrayEncoding : arrayEncoding;
    }
    ...
    private ArrayEncoding getEncoding(final float[] a) {
        return a.length < 63 ? shortArrayEncoding : arrayEncoding;
    }
    ...
    private ArrayEncoding getEncoding(final double[] a) {
        return a.length < 31 ? shortArrayEncoding : arrayEncoding;
    }
    ...
    private class AllArrayEncoding extends LargeFloatingSizePrimitiveTypeEncoding<Object[]> implements ArrayEncoding {
        ...
        AllArrayEncoding(final ProtonBufferEncoderImpl encoder, final ProtonBufferDecoderImpl decoder) {
        ...
        public void writeValue(final boolean[] a) {
            ...
            getEncoder().writeRaw(4 + underlyingEncoder.getConstructorSize() + a.length * underlyingEncoder.getValueSize(null));
    ...
    private class ShortArrayEncoding extends SmallFloatingSizePrimitiveTypeEncoding<Object[]> implements ArrayEncoding {
        ...
        ShortArrayEncoding(final ProtonBufferEncoderImpl encoder, final ProtonBufferDecoderImpl decoder) {
            super(encoder, decoder);
        }
        ...
        public void writeValue(final boolean[] a) {
            BooleanType.BooleanEncoding underlyingEncoder = getUnderlyingEncoding(a);
            getEncoder().writeRaw((byte) (1 + underlyingEncoder.getConstructorSize() + a.length * underlyingEncoder.getValueSize(null)));
    ...
    private BooleanType.BooleanEncoding getUnderlyingEncoding(final boolean[] a) {
        if (a.length == 0) {
            return booleanType.getCanonicalEncoding();
        } else {
            boolean val = a[0];
            for (int i = 1; i < a.length; i++) {
                if (val != a[i]) {
            ...
            return booleanType.getEncoding(val);
        }
    }

    private ByteType.ByteEncoding getUnderlyingEncoding(final byte[] a) {
        return byteType.getCanonicalEncoding();
    }

    private ShortType.ShortEncoding getUnderlyingEncoding(final short[] a) {
        return shortType.getCanonicalEncoding();
    }

    private IntegerType.IntegerEncoding getUnderlyingEncoding(final int[] a) {
        if (a.length == 0 || !allSmallInts(a)) {
            return integerType.getCanonicalEncoding();
        } else {
            return integerType.getEncoding(a[0]);
        }
    }

    private LongType.LongEncoding getUnderlyingEncoding(final long[] a) {
        if (a.length == 0 || !allSmallLongs(a)) {
            return longType.getCanonicalEncoding();
        } else {
            return longType.getEncoding(a[0]);
        }
    }

    private FloatType.FloatEncoding getUnderlyingEncoding(final float[] a) {
        return floatType.getCanonicalEncoding();
    }

    private DoubleType.DoubleEncoding getUnderlyingEncoding(final double[] a) {
        return doubleType.getCanonicalEncoding();
    }

    private CharacterType.CharacterEncoding getUnderlyingEncoding(final char[] a) {
        return characterType.getCanonicalEncoding();
    }

    private static Object[] decodeArray(final ProtonBufferDecoderImpl decoder, final int count) {
        TypeConstructor constructor = decoder.readConstructor();
        return decodeNonPrimitive(decoder, constructor, count);
    }

    private static Object[] decodeNonPrimitive(final ProtonBufferDecoderImpl decoder, final TypeConstructor constructor, final int count) {
        if (count > decoder.getByteBufferRemaining()) {
            throw new IllegalArgumentException(
                "Array element count " + count + " is specified to be greater than the amount of data available (" + decoder.getByteBufferRemaining() + ")");
        }
    ...
    private static Object decodeArrayAsObject(final ProtonBufferDecoderImpl decoder, final int count) {
        TypeConstructor constructor = decoder.readConstructor();
