...
import org.apache.qpid.proton.codec.ProtonBufferDecoder;
import org.apache.qpid.proton.codec.ShortType;
import org.apache.qpid.proton.codec.TypeConstructor;

public class ProtonBufferDecoderImpl implements ProtonBufferDecoder {

    private ProtonBuffer buffer;
    private PrimitiveTypeEncoding[] _constructors = new PrimitiveTypeEncoding[256];
    private Map<Object, DescribedTypeConstructor> _dynamicTypeConstructors = new HashMap<>();

    public ProtonBufferDecoderImpl() {
    }

    ProtonBufferDecoderImpl(final ProtonBuffer buffer) {
        this.buffer = buffer;
    }
    ...
    <V> V readRaw(TypeDecoder<V> decoder, int size) {
        V decode = decoder.decode((ByteBuffer) buffer.slice().setWriteIndex(size));
        buffer.setReadIndex(buffer.getReadIndex() + size);
        return decode;
    }
    ...
    private static class UnknownDescribedType implements DescribedType {
        ...
        public boolean equals(Object obj) {
            ...
            return obj instanceof DescribedType && descriptor == null ? ((DescribedType) obj).getDescriptor() == null
                : descriptor.equals(((DescribedType) obj).getDescriptor()) && described == null ? ((DescribedType) obj).getDescribed() == null
