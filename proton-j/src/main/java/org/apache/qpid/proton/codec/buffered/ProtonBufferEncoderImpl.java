...
import org.apache.qpid.proton.codec.ProtonBufferEncoder;

public final class ProtonBufferEncoderImpl implements ProtonBufferEncoder {
    private static final byte DESCRIBED_TYPE_OP = (byte) 0;

    private ProtonBuffer _buffer;
    ...
    ProtonBufferEncoderImpl(ProtonBuffer buffer, ProtonBufferDecoderImpl decoder) {
        this(decoder);
        setProtonBuffer(buffer);
    }

    public ProtonBufferEncoderImpl(ProtonBufferDecoderImpl decoder) {
        ...
        _arrayType = new ArrayType(this, decoder, _booleanType, _byteType, _shortType, _integerType, _longType, _floatType, _doubleType, _characterType);
    }
    ...
    public void writeCharacter(final char c) {
        // TODO - java character may be half of a pair, should probably throw
        // exception then
    ...
    public void writeDescribedType(final DescribedType d) {
        if (d == null) {
            ...
            _buffer.writeByte(DESCRIBED_TYPE_OP);
    ...
    void writeRaw(byte[] src, int offset, int length) {
        _buffer.writeBytes(src, offset, length);
    }
    ...
    void writeRaw(String string) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            {
                _buffer.writeByte((byte) (0xC0 | ((c >> 6) & 0x1F)));
                _buffer.writeByte((byte) (0x80 | (c & 0x3F)));
            } else if ((c & 0xD800) != 0xD800
                || (c > 0xDBFF)) /*
                                  * U+0800..U+FFFF - excluding surrogate pairs
                                  */
            {
                _buffer.writeByte((byte) (0xE0 | ((c >> 12) & 0x0F)));
                _buffer.writeByte((byte) (0x80 | ((c >> 6) & 0x3F)));
                _buffer.writeByte((byte) (0x80 | (c & 0x3F)));
            } else {
                int low;
                ...
                _buffer.writeByte((byte) (0xF0 | ((c >> 18) & 0x07)));
                _buffer.writeByte((byte) (0x80 | ((c >> 12) & 0x3F)));
                _buffer.writeByte((byte) (0x80 | ((c >> 6) & 0x3F)));
                _buffer.writeByte((byte) (0x80 | (c & 0x3F)));
            }
        }
